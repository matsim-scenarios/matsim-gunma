 ...
 */
public class GunmaTripDashboard implements Dashboard {

	private static final Logger log = LogManager.getLogger(GunmaTripDashboard.class);
	private static final String DIST_GROUP_COLUMN = "dist_group";
	private static final String MAIN_MODE_COLUMN = "main_mode";
		...
		try {
			...
			try (BufferedReader reader = IOUtils.getBufferedReader(groupedRefCsv)) {
				...
				return Arrays.stream(header.split(String.valueOf(c)))
					.filter(s -> !s.equals(MAIN_MODE_COLUMN) && !s.equals(SHARE_COLUMN) && !s.equals(DIST_GROUP_COLUMN))
					.toArray(String[]::new);
			}
	...
	public void configure(Header header, Layout layout, SimWrapperConfigGroup configGroup) {
		...
		makeModalSplitGraphic(first, args);
		...
		first.el(Plotly.class, (viz, data) -> {
			...
			viz.addTrace(BarTrace.builder(Plotly.OBJ_INPUT, Plotly.INPUT).name("Simulated").build(),
				viz.addDataset(data.compute(TripAnalysis.class, "mode_share.csv", args))
					.aggregate(List.of(DIST_GROUP_COLUMN), SHARE_COLUMN, Plotly.AggrFunc.SUM)
					.mapping()
					.x(DIST_GROUP_COLUMN)
					.y(SHARE_COLUMN)
			);
...
//						.rename("share_trips", "share")
		...
		layout.row("second", tab)
			...
			.el(Plotly.class, (viz, data) -> {
				...
				if (modeShareDistRefCsv != null) {
					...
					viz.multiIndex = Map.of(DIST_GROUP_COLUMN, SOURCE_COLUMN);
		...
		layout.row("third", tab)
			...
			.el(Plotly.class, (viz, data) -> {
				...
				if (modeUsersRefCsv != null) {
					...
					viz.multiIndex = Map.of(MAIN_MODE_COLUMN, SOURCE_COLUMN);
					viz.mergeDatasets = true;
				}
	...
	private void makeModalSplitGraphic(Layout.Row first, String[] args) {
		first.el(Plotly.class, (viz, data) -> {
			viz.title = "Modal split";
			viz.description = "Share of trips completed by each mode.";
			...
			Plotly.DataSet ds = viz.addDataset(data.compute(TripAnalysis.class, "mode_share.csv", args))
				.constant(SOURCE_COLUMN, "Simulated")
				.aggregate(List.of(MAIN_MODE_COLUMN), SHARE_COLUMN, Plotly.AggrFunc.SUM);

			if (modeShareRefCsv != null) {
				viz.addDataset(data.resource(modeShareRefCsv))
					.constant(SOURCE_COLUMN, "Reference")
//					.rename("share_trips", "share")
					.aggregate(List.of(MAIN_MODE_COLUMN), SHARE_COLUMN, Plotly.AggrFunc.SUM);
		...
		for (String cat : Objects.requireNonNull(categories, "Categories not set")) {
			...
			layout.row("category_1_" + cat, "By Groups")
				.el(Plotly.class, (viz, data) -> {
					...
					Plotly.DataMapping ds = viz.addDataset(data.computeWithPlaceholder(TripAnalysis.class, "mode_share_per_%s.csv", cat))
						.pivot(List.of(MAIN_MODE_COLUMN, DIST_GROUP_COLUMN, cat), SOURCE_COLUMN, SHARE_COLUMN)
						.aggregate(List.of(MAIN_MODE_COLUMN, SOURCE_COLUMN, cat), SHARE_COLUMN, Plotly.AggrFunc.SUM)
			...
			layout.row("category_2_" + cat, "By Groups")
				...
				.el(Plotly.class, (viz, data) -> {
					...
					Plotly.DataMapping ds = viz.addDataset(data.computeWithPlaceholder(TripAnalysis.class, "mode_share_per_%s.csv", cat))
						.pivot(List.of(MAIN_MODE_COLUMN, DIST_GROUP_COLUMN, cat), SOURCE_COLUMN, SHARE_COLUMN)
						.normalize(List.of(DIST_GROUP_COLUMN, SOURCE_COLUMN, cat), SHARE_COLUMN)
						.rename("sim_share", "Sim")
						.rename("ref_share", "Ref")
						.mapping()
						.name(MAIN_MODE_COLUMN)
